...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
    ...
    case ImGuiCol_nullptr_color: return "nullptr color";
    }
    IM_ASSERT(0);
    return "Unknown";
}
...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
    ...
    if (g.IO.MouseClicked[0])
    {
        if (g.HoveredRootWindow != NULL)
        {
            ...
            {
                if (!(g.HoveredRootWindow->Flags & ImGuiWindowFlags_NoTitleBar))
                {
...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_Border, GetColorU32(ImGuiCol_TitleBg));
    {
        ret = Begin(title, NULL, flags);
    }
    ImGui::PopStyleColor();

    if(!border || no_title) ImGui::PopStyleVar();
...
{
    bool no_title = str_id[0] == '#' && str_id[1] == '#';
    ImGuiWindow* window = GetCurrentWindow();
    bool ret_value = BeginChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);
...
void ImGui::EndChild(const char* name)
{
    ...
    if (window->Flags & ImGuiWindowFlags_ChildTitleBarLine)
    {
        ...
        window->DrawList->AddRectFilled(window->Pos, ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 4), GetColorU32(ImGuiCol_nullptr_color), g.Style.FrameRounding, ImDrawCornerFlags_Top);
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (flags & ImGuiWindowFlags_TitleBarLine)
            window->DrawList->AddRectFilled(title_bar_rect.Min, ImVec2(title_bar_rect.Max.x, title_bar_rect.Min.y + 4.f), GetColorU32(ImGuiCol_nullptr_color), window_rounding, ImDrawCornerFlags_Top);
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
    ...
    {
        pad_r += button_sz;
        close_button_pos = ImVec2(title_bar_rect.Max.x - pad_r - style.FramePadding.x, title_bar_rect.Min.y + 2);
    }
    ...
    RenderTextClipped(layout_r.Min + ImVec2(0, 4.f), layout_r.Max, name, NULL, &text_size, style.WindowTitleAlign, &clip_r);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        {
            ImVec2 clamp_padding = ImMax(style.DisplayWindowPadding, GImGui->Style.DisplaySafeAreaPadding);
        ...
        {
            window->InnerClipRect.Min.x = ImFloor(window->InnerRect.Min.x);
            window->InnerClipRect.Min.y = ImFloor(window->InnerRect.Min.y);
            window->InnerClipRect.Max.x = ImFloor(window->InnerRect.Max.x);
            window->InnerClipRect.Max.y = ImFloor(window->InnerRect.Max.y);
        }

        if (flags & ImGuiWindowFlags_ChildTitleBarLine)
        {
...
bool ImGui::ToggleButtonSelect(const char* label, bool* v, const ImVec2& size_arg, int side, bool KeyBind)
{
    ...
    // Render
    const ImU32 col = KeyBind ? (ImGui::GetColorU32(*v ? ImGuiCol_TitleBg : (hovered ? ImGuiCol_TitleBg : ImGuiCol_Button))) : (ImGui::GetColorU32(*v ? ImGuiCol_TitleBg : (hovered ? ImGuiCol_ChildBg : ImGuiCol_WindowBg)));
    ...
    if (*v)
    {
        ...
        {
        case 0:
            window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + 4.f, bb.Max.y), ImGui::GetColorU32(ImGuiCol_nullptr_color), 0.f);
            break;
        case 1:
            window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + 4.f, bb.Max.y), ImGui::GetColorU32(ImGuiCol_nullptr_color), style.FrameRounding, ImDrawCornerFlags_TopLeft);
            break;
        case 2:
            window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + 4.f, bb.Max.y), ImGui::GetColorU32(ImGuiCol_nullptr_color), style.FrameRounding, ImDrawCornerFlags_BotLeft);
            break;
        }
    }
    if (pressed)
...
bool ImGui::ToggleButton(const char* label, bool* v, const ImVec2& size_arg, int side)
{
    ...
    // Render
    const ImU32 col = ImGui::GetColorU32((hovered && held || *v) ? ImGuiCol_FrameBgActive : (hovered ? ImGuiCol_TitleBg : ImGuiCol_Button));
    //ImGui::RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    switch (side)
    {
    case 0:
...
bool ImGui::ItemsToolTipBeginSettings(const char* name, bool hover)
{
    ImGui::SetNextWindowSize(ImVec2(250, 0));

    OpenPopupOnItemClickKeyBind(name, hover, 0);

    if (!BeginPopup(name, ImGuiWindowFlags_NoMove))  return false;
...
void ImGui::TooltipSettings(const char* text, std::function<void()> func)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImVec2 text_size = CalcTextSize(text, NULL, true);
    ImVec2 plus_size = CalcTextSize("+");
    ImVec2 window_position = window->Pos;
    ...
    ImRect hovered_size = ImRect(ImVec2(end_position.x - button_size.x, window->DC.CursorPos.y), ImVec2(end_position.x, window->DC.CursorPos.y + button_size.y));
    ...
    bool hovered = ImGui::ItemHoverable(hovered_size, id);

    if (hovered) ImGui::SetHoveredID(id);

    const bool clicked = hovered && io.MouseClicked[0];

    if (ImGui::FocusableItemRegister(window, g.ActiveId == id) || clicked) {
    ...
    if (ImGui::ItemsToolTipBeginSettings(text, hovered)) { 
        ImGui::SetWindowPos(ImVec2(hovered_size.Min.x, all_item_size.Max.y));
        if (func) func();
