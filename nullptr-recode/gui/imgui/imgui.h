...
namespace ImGui
{
    ...
    IMGUI_API void          Render(ImDrawList* our_list);                                   // ends the Dear ImGui frame, finalize the draw data. You can get call GetDrawData() to obtain it and run your rendering function (up to v1.60, this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your render function yourself.)
    ...
    // Styles
    IMGUI_API void          set_nullptr_styles(ImGuiStyle* dst = NULL);    // new, recommended style (default)
    ...
    IMGUI_API void          EndChild(const char* name = 0);
    ...
    IMGUI_API void          SetWindowHidden(bool hidden);
    ...
    IMGUI_API void          TextSpace(const char* fmt, ...);
    IMGUI_API void          Text(const char* fmt, ...)                                      IM_FMTARGS(1); // formatted text
    IMGUI_API void          TextVSpace(const char* fmt, va_list args);
    ...
    IMGUI_API bool          KeyBind(const char* label, key_bind_t* k, ImGuiKeyBindFlags flags = 0);

    IMGUI_API bool          ButtonSpace(const char* label, const ImVec2& size_arg = ImVec2(0, 0), bool size_on_window = false);
    IMGUI_API bool          Button(const char* label, const ImVec2& size = ImVec2(0,0), bool size_on_window = false);    // button
    ...
    IMGUI_API bool          BeginComboMulti(const char* label, const char* preview_value, ImGuiComboFlags flags);
    IMGUI_API void          EndCombo(); // only call EndCombo() if BeginCombo() returns true!
    IMGUI_API bool          FunctionCombo(const char* label, const char* preview_value, std::function<void()> func_body);
    ...
    IMGUI_API bool          MultiCombo(const char* name, std::vector<std::string> names, std::vector<bool>* values, int dataSize);
    IMGUI_API bool          MultiCombo(const char* name, std::vector<std::string> names, std::vector<bool*> values, int dataSize);
    IMGUI_API bool          MultiCombo(const char* name, const char** displayName, bool* data, int dataSize);
    ...
    IMGUI_API bool          ColorEdit3(const char* label, color* _color, ImGuiColorEditFlags flags = 0);
    IMGUI_API bool          ColorEdit4(const char* label, color* _color, ImGuiColorEditFlags flags = 0);
    IMGUI_API bool          ColorPicker3(const char* label, color* _color, ImGuiColorEditFlags flags = 0);
    IMGUI_API bool          ColorPicker4(const char* label, color* _color, ImGuiColorEditFlags flags = 0, const float* ref_col = NULL);
    ...
    IMGUI_API void          TooltipSettings(const char* text, std::function<void()> func);
    ...
    IMGUI_API bool          ItemsToolTipBeginSettings(const char* name, bool hover);
...
enum ImGuiWindowFlags_
{
    ...
    // [Internal]
    ImGuiWindowFlags_NavFlattened           = 1 << 21,  // [BETA] Allow gamepad/keyboard navigation to cross over parent border to this child (only use on child that have no scrolling!)
    ImGuiWindowFlags_ChildWindow            = 1 << 22,  // Don't use! For internal use by BeginChild()
    ImGuiWindowFlags_Tooltip                = 1 << 23,  // Don't use! For internal use by BeginTooltip()
    ImGuiWindowFlags_Popup                  = 1 << 24,  // Don't use! For internal use by BeginPopup()
    ImGuiWindowFlags_Modal                  = 1 << 25,  // Don't use! For internal use by BeginPopupModal()
    ImGuiWindowFlags_ChildMenu              = 1 << 26,   // Don't use! For internal use by BeginMenu()
    ImGuiWindowFlags_TitleBarLine           = 1 << 27,
    ImGuiWindowFlags_ChildTitleBarLine      = 1 << 28,
    ImGuiWindowFlags_ListboxTitleBarLine    = 1 << 29,
    ImGuiWindowFlags_NoSpaceAnyItems        = 1 << 30
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_OnSlider            = 1 << 22,
    ImGuiInputTextFlags_OnInput             = 1 << 23
};
...
enum ImGuiKeyBindFlags_
{
    ...
    ImGuiKeyBindFlags_OnTooltip = 1 << 2
};
...
enum ImGuiStyleVar_
{
    ...
    ImGuiStyleVar_ItemInnerSpacing_new,    // ImVec2    ItemInnerSpacing
    ...
    ImGuiStyleVar_MainBarSize,         // float    SelectableTextAlign
    ImGuiStyleVar_COUNT
