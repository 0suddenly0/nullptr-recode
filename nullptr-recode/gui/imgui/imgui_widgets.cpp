...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    const ImVec2 Global_Offset(((flags & ImGuiTextFlags_setSpace && !(window->Flags & ImGuiWindowFlags_NoSpaceAnyItems)) ? 19 + g.Style.ItemInnerSpacing.x : 0), 0);

    const ImVec2 text_pos(window->DC.CursorPos.x + Global_Offset.x, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset + 2.f);
...
void ImGui::TextSpace(const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
void ImGui::TextVSpace(const char* fmt, va_list args)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ImGui::KeyBind(const char* label, key_bind_t* k, ImGuiKeyBindFlags flags)
{
    ...
    ImVec2 button_tooltip_size = CalcItemSize(ImVec2(0, 0), plus_size.x + 8, plus_size.y + 3 * 2.0f);
    ...
    ImVec2 size = ImGui::CalcItemSize(ImVec2(0, 0), ImGui::CalcItemWidth(), label_size.y + 3);
    ...
    ImVec2 render_pos = frame_bb.Min + ImVec2(8, 3);
    ...
    ImVec2 size_button = CalcItemSize(ImVec2(0, 0), key_size.x + 8, key_size.y + 3 * 2.0f);
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_PopupBorderSize, 0);
        {
            ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
            {
                ImGui::PushStyleVar(ImGuiStyleVar_PopupRounding, 4);
                {
                    ...
                    {
                        null_gui::vertical_tabs(k->bind_type, items, true);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec2 Global_offset = ImVec2(((flags & ImGuiButtonFlags_MakeSpace) ? offset_x : 0), 0);

    ImVec2 pos = window->DC.CursorPos + Global_offset;
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
    ImVec2 item_size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ImVec2 size = (flags & ImGuiButtonFlags_SizeOnWindow) ? ImVec2(window->DC.CursorPos.x + (window->Size.x - (offset_x * 2) - offset_scroll) + 7, window->DC.CursorPos.y + item_size.y) : pos + item_size;
    ...
    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_nullptr_color : hovered ? ImGuiCol_nullptr_color : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);    if (flags & ImGuiButtonFlags_noPadY)
    {
...
bool ImGui::ButtonSpace(const char* label, const ImVec2& size_arg, bool size_on_window)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_MakeSpace | ( size_on_window ? ImGuiButtonFlags_SizeOnWindow : 0));
}

bool ImGui::Button(const char* label, const ImVec2& size_arg, bool size_on_window)
{
    return ButtonEx(label, size_arg, size_on_window ? ImGuiButtonFlags_SizeOnWindow : 0);
}
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 bg_col = GetColorU32((held && hovered) ? ImGuiCol_nullptr_color : hovered ? ImGuiCol_nullptr_color : ImGuiCol_Button);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    // Render
    ImU32 col = GetColorU32(held ? ImGuiCol_nullptr_color : ImGuiCol_nullptr_color);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // Render
    ImU32 bg_col = GetColorU32((held && hovered) ? ImGuiCol_nullptr_color : hovered ? ImGuiCol_nullptr_color : ImGuiCol_Button);
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    ImVec2 center = bb.GetCenter();
    if (hovered || held)
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size), inner_rect.Min.y + (window->Flags & ImGuiWindowFlags_ChildTitleBarLine ? 18 : 0), outer_rect.Max.x, inner_rect.Max.y);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    const ImU32 grab_col = GetColorU32(held ? ImGuiCol_nullptr_color : hovered ? ImGuiCol_nullptr_color : ImGuiCol_nullptr_color, alpha);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 check_col = GetColorU32(ImGuiCol_nullptr_color);
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (active)
    {
        ...
        window->DrawList->AddCircleFilled(center, radius - pad, GetColorU32(ImGuiCol_nullptr_color), 16);
    }
...
void ImGui::Separator(float size)
{
    ...
    ImGuiSeparatorFlags flags = ImGuiSeparatorFlags_Horizontal;//(window->DC.LayoutType == ImGuiLayoutType_Horizontal) ? ImGuiSeparatorFlags_Horizontal/*ImGuiSeparatorFlags_Vertical*/ : ImGuiSeparatorFlags_Vertical;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    ImVec2 pos_end = ImVec2(window->DC.CursorPos.x + (window->Size.x - (offset_x * 2) - offset_scroll) + 7, window->DC.CursorPos.y);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);

    const ImRect frame_bb(window->DC.CursorPos + offset, pos_end + ImVec2(0, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect frame_bb_text(window->DC.CursorPos, ImVec2(window->DC.CursorPos.x + (window->Size.x - (offset_x * 3) - offset_scroll) + 7, window->DC.CursorPos.y) + ImVec2(0, label_size.y + style.FramePadding.y * 2.0f));
    ...
    const ImRect total_bb(frame_bb.Min, frame_bb_new.Max);
    ...
    else if (focus_requested || (hovered_input && g.IO.MouseClicked[0]) || g.NavActivateId == id || (g.NavInputId == id && g.TempInputId != id))
    {
    ...
    if (start_text_input || (g.ActiveId == id && g.TempInputId == id))
    {
        ...
        FocusableItemUnregister(window);
        return TempInputScalar(frame_bb_text, id, label, data_type, p_data, format);
    }
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb_new, id, data_type, p_data, p_min, p_max, format, power, ImGuiSliderFlags_None, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);

    //window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(ImVec4(1.f, 1.f, 1.f, 0.4f)));

    // Render grab
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    RenderText(ImVec2(frame_bb.Max.x - value_size_loc.x, frame_bb.Min.y), value_buf);
    window->DrawList->AddRectFilled(frame_bb_new.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_nullptr_color : ImGuiCol_nullptr_color), style.GrabRounding);
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    flags |= ((data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double) ? ImGuiInputTextFlags_CharsScientific : ImGuiInputTextFlags_CharsDecimal) | ImGuiInputTextFlags_OnSlider;
    bool value_changed = false;
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    flags |= ImGuiInputTextFlags_OnInput;  // We call MarkItemEdited() ourselves by comparing the actual data rather than the string.
    ...
    if (p_step != NULL)
    {
        ...
        ImVec2 pos_end = ImVec2(pos_wind.x + (window->Size.x - (offset_x * 2) + 11.f - offset_scroll), window->DC.CursorPos.y + 37.f);
        ...
        PushID(label);
        SetNextItemWidth(size_for_input_text.GetSize().x + 2.f);
        ...
        ImGuiButtonFlags button_flags = ImGuiButtonFlags_Repeat | ImGuiButtonFlags_DontClosePopups | ImGuiButtonFlags_noPadY;
        if (flags & ImGuiInputTextFlags_ReadOnly)
            button_flags |= ImGuiButtonFlags_Disabled;

        window->DC.CursorPos = ImVec2(total_size.Max.x - button_size + 1.f, total_size.Min.y);

        if (ButtonEx("+", ImVec2(button_size - 1.f, 10.f), button_flags))
        {
            DataTypeApplyOp(data_type, '+', p_data, p_data, g.IO.KeyCtrl && p_step_fast ? p_step_fast : p_step);
            value_changed = true;
        }
        window->DC.CursorPos = ImVec2(total_size.Max.x - button_size + 1.f, total_size.Min.y + 10.f + 3.f);
        if (ButtonEx("-", ImVec2(button_size - 1.f, 10.f), button_flags))
        {
            DataTypeApplyOp(data_type, '-', p_data, p_data, g.IO.KeyCtrl && p_step_fast ? p_step_fast : p_step);
            value_changed = true;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (flags & ImGuiInputTextFlags_OnInput)
    {
    ...
    else
    {
        ...
        ImVec2 pos_end = ImVec2(pos_wind.x + (window->Size.x - (offset_x * 2) + 8.f - offset_scroll) + 7, window->DC.CursorPos.y);
    ...
    ImVec2 inner_size = ImVec2(frame_bb_new.Max.x - frame_bb_new.Min.x, frame_size.y);
    if (is_multiline)
    {
        if (!ItemAdd(total_bb, id, &frame_bb))
        {
    ...
    if (!is_multiline)
    {
        ...
        window->DrawList->AddRectFilled(ImVec2(frame_bb_new.Min.x, frame_bb_new.Max.y - 4.f), frame_bb_new.Max, GetColorU32(ImGuiCol_nullptr_color), style.FrameRounding, ImDrawCornerFlags_Bot);
    }

    const ImVec4 clip_rect(frame_bb_new.Min.x, frame_bb_new.Min.y, frame_bb_new.Min.x + inner_size.x, frame_bb_new.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb_new.Min.x, frame_bb.Min.y), label);
...
bool ImGui::ColorEdit3(const char* label, color* _color, ImGuiColorEditFlags flags)
{
    return ColorEdit4(label, _color, flags | ImGuiColorEditFlags_AlphaBar);
}
...
bool ImGui::ColorEdit4(const char* label, color* _color, ImGuiColorEditFlags flags)
{
    ...
    const ImVec2 pos = ImVec2(window->Pos.x + window->Size.x - w_button, window->DC.CursorPos.y);

    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        const float text_offset_x = (flags & ImGuiColorEditFlags_NoInputs) ? w_button : w_full + style.ItemInnerSpacing.x;
        window->DC.CursorPos.x += 19 + style.ItemInnerSpacing.x;//ImVec2(pos.x + text_offset_x, pos.y + style.FramePadding.y);
        window->DC.CursorPos.y += 1;//ImVec2(pos.x + text_offset_x, pos.y + style.FramePadding.y);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                //SetNextWindowSize(ImVec2(200, 0));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(7, 7));
        {
            ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(2, 2));
            {
                ...
                {
                    SetNextWindowSize(ImVec2((flags & ImGuiColorEditFlags_AlphaBar) && !(flags & ImGuiColorEditFlags_NoAlpha) ? 176 : 156, 0));
                    if (BeginPopup("picker"))
                    {
                        ...
                        SetNextItemWidth(square_sz * 9.f); // Use 256 + bar sizes?
                        value_changed |= ColorPicker4("##picker", _color, picker_flags, &g.ColorPickerRef.x);
                        EndPopup();
                    }
                }
                ImGui::PopStyleVar();
    ...
    if (value_changed && picker_active_window == NULL)
    {
        ...
        *col[2] = f[2];
        ...
        //*_color = color(*col[0], *col[1], *col[2], *col[3]);
    ...
    _color->inalizate_char(*col[0], *col[1], *col[2], *col[3]);

    /*_color->get<int>(return_color::r) = *col[0] * 255f;
    _color->get<int>(return_color::g) = *col[1] * 255;
    _color->get<int>(return_color::b) = *col[2] * 255.f;
    _color->get<int>(return_color::a) = *col[3] * 255.f;*/

    return value_changed;
}

bool ImGui::ColorPicker3(const char* label, color* _color, ImGuiColorEditFlags flags)
{
    color col4 = *_color;
...
bool ImGui::ColorPicker4(const char* label, color* _color, ImGuiColorEditFlags flags, const float* ref_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    float sv_picker_size = ImMax(bars_width * 1, width - /*(alpha_bar ? 2 : 1)*/2 * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
    ...
    {
        PushItemWidth(/*(alpha_bar ? bar1_pos_x : bar0_pos_x)*/bar1_pos_x + bars_width - picker_pos.x);
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            if (ColorEdit4("##rgb", _color, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            value_changed |= ColorEdit4("##hsv", _color, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            value_changed |= ColorEdit4("##hex", _color, sub_flags | ImGuiColorEditFlags_DisplayHex);
        PopItemWidth();
    }
...
void ImGui::ColorEditOptionsPopup(float* col, ImGuiColorEditFlags flags)
{
    ...
    if (Button("paste", ImVec2(50, 0)))
    {
        ...
        col[1] = color_copy[1];
        col[2] = color_copy[2];
        if (flags & ImGuiColorEditFlags_AlphaBar) col[3] = color_copy[3];
    }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    {
        // Framed type
        const ImU32 bg_col = GetColorU32((held && hovered) ? ImGuiCol_nullptr_color : hovered ? ImGuiCol_nullptr_color : ImGuiCol_Header);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    bool text_offset = selected || hovered;
    ...
    if (hovered || selected)
    {
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_Border : hovered ? ImGuiCol_Border : ImGuiCol_Border);
    ...
    RenderTextClipped(text_min + ImVec2(text_offset ? 6 : 2, 0), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb_enlarged);
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    const float global_offset_float = 19 + style.ItemInnerSpacing.x;
    ...
    ImVec2 pos_end = ImVec2(pos_wind.x + (window->Size.x - (global_offset_float * 2) + 15.f - offset_scroll), window->DC.CursorPos.y);
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0, 0));
    {
        ...
        if (label_size.x > 0)
        {
            ...
            window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb_old.Min.y), ImVec2(frame_bb.Max.x, frame_bb_old.Min.y + 4), GetColorU32(ImGuiCol_nullptr_color), style.FrameRounding, ImDrawCornerFlags_Top);
        ...
        BeginChildFrame(id, frame_bb.GetSize(), ImGuiWindowFlags_ListboxTitleBarLine);
    }
    ImGui::PopStyleVar();
