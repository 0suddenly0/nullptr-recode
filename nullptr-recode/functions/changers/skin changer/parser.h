...
namespace valve_parser {
	...
	public:
		...
		static char32_t* end_reached(char32_t* p, char32_t expectedTag) {
			for (; *p; p++) {
				...
				if (*p == expectedTag) return p;
				else return (char32_t*)(-1);
			}
	...
	public:
		...
		std::shared_ptr<node> breadth_first_search(char* name, bool caseInsensitive = false) const {
			...
			while (!q.empty()) {
				...
				if (f->to_key_value()) {
					if (!caseInsensitive && util::str_equ(f->to_key_value()->key, name)) return f;
					if (caseInsensitive && util::str_equ_i(f->to_key_value()->key, name)) return f;
				}
				if (f->to_object()) {
					if (!caseInsensitive && util::str_equ(f->to_object()->name, name)) return f;
					if (caseInsensitive && util::str_equ_i(f->to_object()->name, name)) return f;
				}
		...
		std::vector<std::shared_ptr<node>> breadth_first_search_multiple(char* name, bool caseInsensitive = false) const {
			...
			while (!q.empty()) {
				...
				if (f->to_key_value()) {
					if (!caseInsensitive && util::str_equ(f->to_key_value()->key, name)) vec.push_back(f);
					if (caseInsensitive && util::str_equ_i(f->to_key_value()->key, name)) vec.push_back(f);
				}
				if (f->to_object()) {
					if (!caseInsensitive && util::str_equ(f->to_object()->name, name)) vec.push_back(f);
					if (caseInsensitive && util::str_equ_i(f->to_object()->name, name)) vec.push_back(f);
				}
		...
		bool load(char* path, encoding encoding) {
			...
			fseek(f, 0, SEEK_SET);
			if (fread(text, 1, size, f) != size) return false;
	...
	private:
		char* skip_bom(char* p, const encoding encoding) const {
			if (!memcmp(p, boms_[encoding].str, boms_[encoding].len)) return p + boms_[encoding].len;
			else return p;
		}
		...
		static size_t cnv_utf16_be_len(char* text) {
			size_t len = 0;
			auto u16_text = (char16_t*)(text);
			while (*u16_text) {

				char16_t c = 0;
				const auto t = (char*)(u16_text);
				c |= char16_t(*(t + 1)) << 8;
				c |= *t;

				//4 bytes
				if (c >= 0xD800 && c <= 0xDFFF) u16_text++;
		...
		static size_t cnv_utf16_le_len(char* text) {
			...
			auto u16_text = (char16_t*)(text);
			while (*u16_text) {
				if (*u16_text >= 0xD800 && *u16_text <= 0xDFFF) u16_text++;
		...
		bool identify(std::shared_ptr<node>& node) {
			...
			if (!object_open && !val_start) return false;
