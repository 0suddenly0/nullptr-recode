...
class c_cstrike15_item_schema;
class c_cstrike15_item_system;
...
int get_weapon_rarity(std::string rarity) {
...
auto get_export(const char* module_name, const char* export_name) -> void* {
	...
	while (!((mod = GetModuleHandleA(module_name))))
		Sleep(100);
	return (void*)(GetProcAddress(mod, export_name));
}

namespace skin_parser {
	bool skins_parsed = false;
	std::vector<paint_kit> skins;
	std::vector<paint_kit> gloves;
	std::vector<paint_kit> stickers;
			...
			if (rarity->to_object()) {
				for (const auto& child : rarity->children) {
					if (child->to_key_value()) {
						...
						skinInfo->rarity = get_weapon_rarity(paint_kit_rarity);
					}
				}
			}
		}
		...
		for (auto weapon : weaponNames) {
			for (auto skin : weaponSkins[weapon]) {
				if (skinMap[skin].paintkit < 10000) {
					...
					if (!skinNames[skinMap[skin].tag_name].empty()) skins.push_back(info);
				} else {
					paint_kit info;
					info.id = skinMap[skin].paintkit;
					info.weaponName.push_back(weapon);
