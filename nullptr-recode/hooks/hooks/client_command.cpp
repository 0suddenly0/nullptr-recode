...
namespace hooks {
	void __fastcall client_command::hook(void* ecx, void* edx, const char* command) {
		...
		std::wstring new_cmd = utils::s2ws(std::string(command));
		int text_size = std::string(command).size();
		int size = strstr(command, "say ") ? 6 : strstr(command, "say_team ") ? 11 : 0;

		if (settings::misc::chat_filter_bypass && ((strstr(command, "say ") && text_size > size) || (strstr(command, "say_team ") && text_size > size)) && command[size == 6 ? 5 : 10] != '!') {
