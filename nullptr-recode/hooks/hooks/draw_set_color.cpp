...
namespace hooks {
	void __stdcall draw_set_color::hook(int r, int g, int b, int a) {
		...
		static const auto return_to_scope_arc = utils::pattern_scan(GetModuleHandleW(L"client.dll"), "6A 00 FF 50 3C 8B 0D ? ? ? ? FF B7 ? ? ? ?") + 0x5;
		static const auto return_to_scope_lens = utils::pattern_scan(GetModuleHandleW(L"client.dll"), "FF 50 3C 8B 4C 24 20") + 0x3;
