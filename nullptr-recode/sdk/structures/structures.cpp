...
bool c_base_entity::new_setup_bones(matrix3x4* matrix)
{
	if (this == sdk::local_player)
	{
		const auto Backup = *(int*)(uintptr_t(this) + ptrdiff_t(0x272));

		*(int*)(uintptr_t(this) + ptrdiff_t(0x272)) = -1;

		this->setup_bones(matrix, 126, 0x00000100 | 0x200, sdk::global_vars->curtime);
	...
	{
		*reinterpret_cast<int*>(uintptr_t(this) + 0xA30) = sdk::global_vars->framecount;
...
matrix3x4 c_base_entity::get_bone_matrix(int BoneID)
{
	matrix3x4 matrix;
...
c_utl_vector<c_ref_counted*>& c_base_combat_weapon::custom_materials() {
...
c_user_cmd*& c_base_player::current_command() {
...
vec3& c_base_player::get_abs_angles2()
{
	return call_vfunction<vec3 & (__thiscall*)(void*)>(this, 11)(this);
}
vec3& c_base_player::get_abs_origin()
{
	return call_vfunction<vec3 & (__thiscall*)(void*)>(this, 10)(this);
}


void c_base_player::set_angle2(vec3 wantedang)
{
...
c_animation_layer* c_base_player::get_anim_overlays() {
	return *(c_animation_layer**)((DWORD)this + 0x2980);
}

c_animation_layer* c_base_player::get_anim_overlay(int i) {
	if (i < 15)
...
vec3 c_base_player::get_hitbox_pos(int hitbox_id) {
...
bool c_base_player::get_optimized_hitbox_pos(int hitbox, vec3& output)
{
...
vec3 c_base_player::get_bone_pos(int bone) {
	matrix3x4 boneMatrix[MAXSTUDIOBONES];
...
bool c_base_player::can_see_player(c_base_player* player, const vec3& pos) {
	c_game_trace tr;
	ray_t ray;
	c_trace_filter filter;
	filter.pSkip = this;
...
float c_base_player::can_see_player(const vec3& pos)
{
	c_game_trace tr;
	ray_t ray;
	c_trace_filter filter;
