...
namespace sdk {
	...
	c_network_string_table_container* network_string_table = nullptr;
	...
	void initialize() {
		...
		network_string_table = get_interface<c_network_string_table_container>(engine_factory, "VEngineClientStringTable001");
...
#define PRINT_INTERFACE(name) utils::console_print("[interface] %-20s: %p\n", STRINGIFY(name), name)
		...
		PRINT_INTERFACE(network_string_table);
	}
}
