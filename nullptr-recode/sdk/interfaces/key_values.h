...
class c_key_values {
public:
	void init_key(const char* key_name) {
	...
	void load_from_buffer_key(const char* resource_name, const char* buf, void* file_sys = nullptr, const char* path_id = nullptr, void* eval_sym_proc = nullptr, void* unk = nullptr) {
	...
	void set_int_key(const char* name, int value) {
	...
	void set_string_key(const char* name, const char* a2) {
		auto key = find_key(name, true);
		if (!key) return;
