...
public:
    virtual bool should_hit_entity(c_handle_entity*pEntity, int contentsMask) = 0;
...
struct ray_t
{
    ...
    veca  m_Extents;     // Describes an axis aligned box extruded along a ray
    ...
    void init(vec3 const& start, vec3 const& end)
    {
        ...
        m_Extents = vec3(0, 0, 0);

        m_pWorldAxisTransform = NULL;
        m_IsRay = true;

        // Offset m_Start to be in the center of the box...
        m_StartOffset = vec3(0, 0, 0);
        m_Start = start;
    }
...
public:
    ...
    virtual int   get_point_contents_collideable(c_collideable *pCollide, const vec3 &vecAbsPosition) = 0;
    virtual void  clip_ray_to_entity(const ray_t &ray, unsigned int fMask, c_handle_entity*pEnt, c_game_trace *pTrace) = 0;
    virtual void  clip_ray_to_collideable(const ray_t &ray, unsigned int fMask, c_collideable*pCollide, c_game_trace*pTrace) = 0;
    virtual void  trace_ray(const ray_t &ray, unsigned int fMask, c_base_trace_filter*pTraceFilter, c_game_trace*pTrace) = 0;
};
