...
class c_map_overview {
public:
	virtual	~c_map_overview(void) {};
	...
	virtual void set_center(vec2 &mappos) = 0; // set map pos in center of panel
	virtual void set_player_positions(int index, const vec3& position, const qangle& angle) = 0; // update player position
	virtual vec2 world_to_map(vec3 &worldpos) = 0; // convert 3d world to 2d map pos
...
public:
	...
	virtual c_material*           trace_line_material_and_lighting(const vec3&start, const vec3&end, vec3&diffuseLightColor, vec3& baseColor) = 0;
	...
	virtual int                   is_box_visible(const vec3& mins, const vec3& maxs) = 0;
	virtual int                   ss_Box_inview_cluster(const vec3& mins, const vec3& maxs) = 0;
	virtual bool                  cull_box(const vec3& mins, const vec3& maxs) = 0;
	...
	virtual void                  get_ambient_light_color(vec3& color) = 0;
	virtual int                   get_dx_support_level() = 0;
	virtual bool                  supports_hdr() = 0;
	virtual void                  mat_stub(c_material_system*pMatSys) = 0; // 50
	...
	virtual struct c_voice_tweak_s* get_voice_tweak_api(void) = 0;
	...
	virtual bool                  does_box_touch_area_frustum(const vec3&mins, const vec3&maxs, int iArea) = 0; // 60
	...
	virtual void                  compute_lighting(const vec3& pt, const vec3* pNormal, bool bClamp, vec3& color, vec3*pBoxColors = NULL) = 0;
	virtual void                  activate_occluder(int nOccluderIndex, bool bActive) = 0;
	virtual bool                  is_occluded(const vec3&vecAbsMins, const vec3&vecAbsMaxs) = 0; // 74
	...
	virtual void                  debug_draw_phys_collide(const c_phys_collide *pCollide, c_material *pMaterial, const matrix3x4& transform, const uint8_t* color) = 0; //79
	...
	virtual int                   is_skybox_visible_from_point(const vec3&vecPoint) = 0;
	...
	virtual vec3                get_light_for_point_fast(const vec3&pos, bool bClamp) = 0;
	...
	virtual void                  set_achievement_mgr(c_achievement_mgr *pAchievementMgr) = 0;
	virtual c_achievement_mgr*      get_achievement_mgr() = 0;
	...
	virtual c_sp_shared_memory*      get_single_player_shared_memory_space(const char *szName, int ent_num = (1 << 11)) = 0;
	virtual void                  compute_lighting_cube(const vec3& pt, bool bClamp, vec3*pBoxColors) = 0;
	...
	virtual void                  solid_moved(class c_client_entity *pSolidEnt, class c_collideable *pSolidCollide, const vec3* pPrevAbsOrigin, bool accurateBboxTriggerChecks) = 0;
	virtual void                  trigger_moved(class c_client_entity *pTriggerEnt, bool accurateBboxTriggerChecks) = 0;
	virtual void                  compute_leaves_connected(const vec3&vecOrigin, int nCount, const int *pLeafIndices, bool *pIsConnected) = 0;
	...
	//virtual void                trace_paint_surface( const model_t *model, const Vector& position, float radius, c_utl_vector<Color>& surfColors ) = 0;
	virtual void                  sphere_trace_paint_surface(const model_t* model, const vec3& position, const vec3&vec2, float radius, /*c_utl_vector<unsigned char, c_utl_memory<unsigned char, int>>*/ int& utilVecShit) = 0;
