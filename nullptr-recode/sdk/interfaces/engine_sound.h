...
public:
	...
	virtual int emit_sound(c_recipient_filter& filter, int ient_index, int iChannel, const char *pSoundEntry, unsigned int nSoundEntryHash, const char *pSample,
		float flVolume, float flAttenuation, int nSeed, int iFlags = 0, int iPitch = PITCH_NORM,
		const vec3 *pOrigin = NULL, const vec3 *pDirection = NULL, c_utl_vector< vec3 >* pUtlVecOrigins = NULL, bool bUpdatePositions = true, float soundtime = 0.0f, int speakerentity = -1) = 0;

	virtual int emit_sound(c_recipient_filter& filter, int ient_index, int iChannel, const char *pSoundEntry, unsigned int nSoundEntryHash, const char *pSample,
		float flVolume, soundlevel_t iSoundlevel, int nSeed, int iFlags = 0, int iPitch = PITCH_NORM,
		const vec3 *pOrigin = NULL, const vec3 *pDirection = NULL, c_utl_vector< vec3 >* pUtlVecOrigins = NULL, bool bUpdatePositions = true, float soundtime = 0.0f, int speakerentity = -1) = 0;

	virtual void emit_sentence_by_index(c_recipient_filter& filter, int ient_index, int iChannel, int iSentenceIndex,
		float flVolume, soundlevel_t iSoundlevel, int nSeed, int iFlags = 0, int iPitch = PITCH_NORM,
		const vec3 *pOrigin = NULL, const vec3 *pDirection = NULL, c_utl_vector< vec3 >* pUtlVecOrigins = NULL, bool bUpdatePositions = true, float soundtime = 0.0f, int speakerentity = -1) = 0;
	...
	virtual void    set_room_type(c_recipient_filter& filter, int roomType) = 0;
	virtual void    set_player_dsp(c_recipient_filter& filter, int dspType, bool fastReset) = 0;
