...
class c_client_material_system;
class c_paint_material;
class c_paint_map_data_manager;
class c_paint_map_texture_manager;
class gpu_memory_stats;
struct aspect_ratio_info_t;
struct cascaded_shadow_mapping_state_t;

class c_material_proxy_factory;
class c_texture;
...
public:
    ...
    virtual c_material_system_hardware_config*  get_hardware_config(const char *pVersion, int *returnCode) = 0;
    ...
    virtual int                             get_shaders(int nFirstShader, int nMaxCount, c_shader **ppShaderList) const = 0;
    ...
    virtual c_material_proxy_factory*       get_material_proxy_factory() = 0;
    virtual void                            set_material_proxy_factory(c_material_proxy_factory* pFactory) = 0;
    ...
    virtual c_texture*                       create_procedural_texture(const char	*pTextureName, const char *pTextureGroupName, int w, int h, image_format fmt, int nFlags) = 0;
    ...
    virtual c_texture*                       create_render_target_texture(int w, int h, render_target_size_mode_t sizeMode, image_format	format, material_render_target_depth_t depth = MATERIAL_RT_DEPTH_SHARED) = 0;
