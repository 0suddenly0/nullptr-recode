...
public:
    ...
    virtual c_material_var*          find_var(const char *varName, bool *found, bool complain = true) = 0;
...
public:
    ...
    virtual void                    set_view_target(const c_studio_hdr *pStudioHdr, int nBodyIndex, const vec3& target) = 0;
    virtual model_instance_handle_t create_instance(c_client_renderable*pRenderable, light_cache_handle_t *pCache = NULL) = 0;
    ...
    virtual bool                    change_instance(model_instance_handle_t handle, c_client_renderable*pRenderable) = 0;
    virtual void                    add_decal(model_instance_handle_t handle, ray_t const& ray, vec3 const& decalUp, int decalIndex, int body, bool noPokeThru, int maxLODToDecal) = 0;
    ...
    virtual matrix3x4*            draw_model_shadow_setup(c_client_renderable*pRenderable, int body, int skin, draw_model_info_t *pInfo, matrix3x4 *pCustomBoneToWorld = NULL) = 0;
    virtual void                    draw_model_shadow(c_client_renderable*pRenderable, const draw_model_info_t &info, matrix3x4 *pCustomBoneToWorld = NULL) = 0;
    ...
    virtual bool                    draw_model_setup(model_render_info_t &pInfo, draw_model_state_t *pState, matrix3x4 **ppBoneToWorldOut) = 0;
    virtual void                    draw_model_execute(c_client_renderable* ctx, const draw_model_state_t& state, const model_render_info_t& pInfo, matrix3x4* pCustomBoneToWorld = NULL) = 0;
    virtual void                    setup_lighting(const vec3 &vecCenter) = 0;
    ...
    virtual void                    setup_lighting_ex(const vec3&vecCenter, model_instance_handle_t handle) = 0;
    virtual bool                    get_brightest_shadowing_light_source(const vec3&vecCenter, vec3& lightPos, vec3& lightBrightness, bool bAllowNonTaggedLights) = 0;
    virtual void                    compute_lighting_state(int nCount, const lighting_query_t *pQuery, material_lighting_state_t *pState, c_texture **ppEnvCubemapTexture) = 0;
    ...
    virtual void                    compute_static_lighting_state(int nCount, const static_lighting_query_t *pQuery, material_lighting_state_t *pState, material_lighting_state_t *pDecalState, color_mesh_info_t **ppStaticLighting, c_texture **ppEnvCubemapTexture, data_cache_handle_t *pColorMeshHandles) = 0;
