...
class glow_object_definition {
public:
    glow_object_definition() { memset(this, 0, sizeof(*this)); }

    class c_client_entity* m_pEntity;    //0x0000
    union
    {
        vec3 m_vGlowColor;           //0x0004
        struct
        {
    ...
    bool is_unused() const { return m_nNextFreeSlot != glow_object_definition::ENTRY_IN_USE; }
...
public:
    int register_glow_object(c_client_entity *pEntity, const vec3 &vGlowColor, float flGlowAlpha, bool bRenderWhenOccluded, bool bRenderWhenUnoccluded, int nSplitScreenSlot) {
        ...
        m_GlowObjectDefinitions[nIndex].m_nNextFreeSlot = glow_object_definition::ENTRY_IN_USE;
    ...
    c_utl_vector<glow_object_definition> m_GlowObjectDefinitions; //0x0000
