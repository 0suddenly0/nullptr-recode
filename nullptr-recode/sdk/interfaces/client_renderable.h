...
public:
	...
	virtual bool                       setup_bones(matrix3x4 *pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime) = 0;
	virtual void                       setup_weights(const matrix3x4 *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights, float *pFlexDelayedWeights) = 0;
	...
	virtual void                       get_shadow_render_bounds(vec3&mins, vec3 &maxs, int /*ShadowType_t*/ shadowType) = 0;
	...
	virtual bool                       get_shadow_cast_direction(vec3*pDirection, int /*ShadowType_t*/ shadowType) const = 0;
	...
	virtual bool                       get_attachment(int number, vec3 &origin, qangle &angles) = 0;
